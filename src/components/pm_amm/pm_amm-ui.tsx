...
export function PmAmmCreate() {
  const { initialize } = usePmAmmProgram()
      ...
      {accounts.isLoading ? (
        ...
        <div className="grid md:grid-cols-2 gap-4">
          {accounts.data?.map((account) => (
            <PmAmmCard key={account.publicKey.toString()} account={account.publicKey} />
          ))}
        </div>
      ) : (
...
function PmAmmCard({ account }: { account: PublicKey }) {
  const { accountQuery, incrementMutation, setMutation, decrementMutation, closeMutation } = usePmAmmProgramAccount({
    account,
  })
